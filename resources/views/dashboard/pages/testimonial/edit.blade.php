@extends('dashboard.layouts.app')
@section('title', __('admin.edit'))
@section('content')
    <div class="layout-px-spacing">
        <div class="row layout-top-spacing">
            <div id="basic" class="col-lg-12 layout-spacing">
                <div class="statbox widget box box-shadow">
                    <div class="widget-header">
                        <div class="row mb-4">
                            <div class="col-12 d-flex justify-content-between align-items-center">
                                <h4 class="">{{ __('admin.edit') }}</h4>
                                <x-back-button route="{{ route('admin.testimonials.index') }}" />
                            </div>
                        </div>
                    </div>
                    <div class="widget-content widget-content-area">
                        <div class="row">
                            <div class="col-lg-12 col-12 mx-auto">
                                <form action="{{ route('admin.testimonials.update', $testimonial->id) }}" method="POST">
                                    @csrf
                                    <div class="form-group">
                                        <div class="row">
                                            <div class="col-6 mb-3">
                                                <label for="name"
                                                    class="text-dark">{{ __('admin.name') }}</label>
                                                <input type="text" name="name" id="name" class="form-control"
                                                    value="{{ old('name', $testimonial->getTranslation('name', 'ar')) }}">
                                            </div>
                                            <div class="col-6 mb-3">
                                                <label for="job_title"
                                                    class="text-dark">{{ __('admin.job_title') }}</label>
                                                <input type="text" name="job_title" id="job_title" class="form-control"
                                                    value="{{ old('job_title', $testimonial->getTranslation('job_title', 'ar')) }}">
                                            </div>
                                            <div class="col-12 mb-3">
                                                <label for="review"
                                                    class="text-dark">{{ __('admin.review') }}</label>
                                                <textarea name="review" id="review" class="form-control"
                                                    rows="3">{{ old('review', $testimonial->getTranslation('review', 'ar')) }}</textarea>
                                            </div>
                                            <div class="col-6 mb-3">
                                                <label for="rate"
                                                    class="text-dark">{{ __('admin.rate') }}</label>
                                                <input type="number" min="1" max="5" name="rate" id="rate" class="form-control"
                                                    value="{{ old('rate', $testimonial->rate) }}">
                                            </div>
                                            <div class="col-6 mb-3">
                                                <label for="statusInput" class="text-dark">{{ __('admin.status') }}</label>
                                                <select name="status" id="statusInput" class="form-control">
                                                    @foreach ($status as $stat)
                                                        <option @selected($stat->value == old('status', $testimonial->status->value))
                                                            value="{{ $stat->value }}">
                                                            {{ $stat->lang() }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-12">
                                            <input type="submit" value="{{ __('admin.update') }}"
                                                class="mt-4 btn btn-primary">
                                        </div>
                                    </div>
                                </form>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection