...
<nav class="breadcrumb-one" aria-label="{{ __('admin.breadcrumb') }}">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page"><span>{{ __('admin.create_user_shipping_price') }}</span></li>
    </ol>
</nav>
@endpush
@section('content')
                ...
                <div class="widget-content widget-content-area">
                    <div class="row">
                        <div class="col-lg-12 col-12 mx-auto">
                            <form action="{{ route('admin.user-shipping-prices.store', $user->id) }}" method="POST"
                                enctype="multipart/form-data">
                                @csrf

                                <!-- Hidden fields for company data -->
                                <input type="hidden" name="company_id" id="company_id">
                                <input type="hidden" name="company_name_ar" id="company_name_ar">
                                <input type="hidden" name="company_name_en" id="company_name_en">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function handleCompanySelection(company) {
            ...
            if (countryDisplay) {
                if (company.country) {
                    let countryName = 'N/A';
                    if (company.country.name && typeof company.country.name === 'object') {
                        countryName = currentLocale === 'ar' ?
                            (company.country.name.ar || company.country.name.name_ar || '') :
                            (company.country.name.en || company.country.name.name_en || '');
                    } else if (company.country.name_ar && company.country.name_en) {
                        countryName = currentLocale === 'ar' ? company.country.name_ar : company.country.name_en;
                    } else if (company.country.name) {
                        countryName = company.country.name;
                    }

                    countryDisplay.textContent = countryName || company.country.name_en || company.country.name || 'N/A';
                } else {
