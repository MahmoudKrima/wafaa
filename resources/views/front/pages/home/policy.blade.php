                        ...
                        @if($policy && $policy->policy_description)
                            <div>
                                <div class="terms-text">
                                    {!! $policy->getTranslation('policy_description', app()->getLocale()) !!}
                                </div>
                                <div>
                                    <h3 class="mb-2"></h3>
                                    <p class="mb-0 opacity-75">
                                        {{ __('admin.last_updated') ?? 'آخر تحديث' }}:
                                        {{ $policy->updated_at ? $policy->updated_at->format('Y-m-d') : 'غير محدد' }}
                                    </p>
                                </div>
                            </div>
                        @else
                            <div>
                                ...
                                <h4 class="text-muted mb-3">
                                    {{ __('admin.no_policy_available') ?? 'لا توجد سياسة خصوصية متاحة حالياً' }}</h4>
                                <p class="text-muted mb-4">
                                    {{ __('admin.policy_coming_soon') ?? 'سيتم إضافة سياسة الخصوصية قريباً' }}</p>
