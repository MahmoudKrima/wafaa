                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <input id="phoneInput" type="text" placeholder="05XXXXXXXX"
                                                name="phone" placeholder="{{ __('admin.phone') }}" style="direction:ltr;"
...
<script>
    $(document).ready(function() {
        ...
        const existingShippingCompanies = @json($reciever->shippingCompanies ?? []);
        console.log('Existing shipping companies:', existingShippingCompanies);

        // If there's old data (validation errors), restore it
        if (oldShippingCompanies && oldShippingCompanies.length > 0) {
            console.log('Restoring old data:', oldShippingCompanies);
            oldShippingCompanies.forEach(function(shippingCompany, index) {
                ...
                if (shippingCompany.company_id) {
                    console.log('Setting old company:', shippingCompany.company_id, 'City:', shippingCompany.city_id);
        ...
        } else if (existingShippingCompanies && existingShippingCompanies.length > 0) {
            console.log('Loading existing shipping companies:', existingShippingCompanies);
            existingShippingCompanies.forEach(function(shippingCompany, index) {
                addShippingCompanyRow();
                const lastRow = $('.shipping-company-row').last();
                const companySelect = lastRow.find('.shipping-company-select');
                const citySelect = lastRow.find('.city-select');

                // Check both possible field names for company ID
                const companyId = shippingCompany.shipping_company_id || shippingCompany.company_id;
                const cityId = shippingCompany.city_id;

                if (companyId) {
                    console.log('Setting existing company:', companyId, 'City:', cityId);
                    companySelect.val(companyId);

                    if (cityId) {
                        citySelect.data('pending-city-value', cityId);
                        console.log('Set pending city value:', cityId);
                    }

                    // Add a delay for each row to ensure proper loading
                    setTimeout(function() {
                        console.log('Triggering change for company:', companyId);
                        companySelect.trigger('change');
        ...
        $(document).on('change', '.shipping-company-select', function() {
            ...
            if (companyId) {
                ...
                const baseUrl = @json(route('user.recievers.getCitiesByCompanyAndCountry', ['shippingCompanyId' => '__COMPANY_ID__']));
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (currentCityValue) {
                            console.log('Restoring city value:', currentCityValue, 'Available options:', citySelect.find('option').length);
                            console.log('Available city options:', citySelect.find('option').map(function() {
                                return $(this).val();
                            }).get());
                            ...
                            setTimeout(function() {
                                ...
                                if (citySelect.val() !== currentCityValue) {
                                    console.log('Failed to set city value, trying alternative matching...');
                                    // Try to find by exact match first
                                    const exactMatch = citySelect.find('option[value="' + currentCityValue + '"]');
                                    if (exactMatch.length > 0) {
                                        citySelect.val(currentCityValue);
                                        console.log('Found exact match and set city value');
                                    } else {
                                        ...
                                        allOptions.each(function() {
                                            ...
                                            if (optionValue && optionValue.toString().includes(currentCityValue.toString().substring(0, 8))) {
                                                ...
                                                console.log('Found partial match and set city value:', optionValue);
                                                found = true;
                                    ...
                                    console.log('City value successfully set to:', citySelect.val());
                                }
