(() => {
    const SAUDI_ID = "65fd1a1c1fdbc094e3369b2a";
    ...
    function hideDevButtons() {
        document
            .querySelectorAll(
                '#existing_receiver_section button[onclick*="test"]'
            )
    ...
    function canAdd(item) {
        ...
        if (window.selectedReceivers.some(dup)) {
            toast(
                t("receiver_already_selected", "Receiver already selected"),
                "error"
            );
            return false;
        }
            ...
            if (!btn.dataset.bound) {
                btn.addEventListener("click", function () {
                    ...
                    if (
                        removed &&
                        !removed.isNew &&
                        initialExistingMap[removed.id]
                    ) {
                        const sel = document.getElementById("receiver_select");
                        if (
                            sel &&
                            !sel.querySelector(`option[value="${removed.id}"]`)
                        ) {
                            const opt = document.createElement("option");
    ...
    function loadCountriesForLocalShipping() {
        ...
        fetchJson(
            ...
            .then((data) => {
                const list = (data && data.results) || [];
                countryField.innerHTML = `<option value="">${t("select_country", "Select Country")}</option>`;
            ...
            .catch(() => {
                countryField.innerHTML = `<option value="">${t("error_loading_countries", "Error loading countries")}</option>`;
                countryField.disabled = true;
            });
    }
