(() => {
    ...
    const API = {
      ...
      states: (countryId, shippingCompanyId) =>
        ...
        `https://ghaya-express-staging-af597af07557.herokuapp.com/v1/states?pageSize=500&page=0&countryId=${encodeURIComponent(countryId)}&shippingCompanyId=${encodeURIComponent(shippingCompanyId)}`,
      cities: (countryId, stateId, shippingCompanyId) =>
        (window.API_ENDPOINTS?.cities?.(countryId, stateId, shippingCompanyId)) ||
        `https://ghaya-express-staging-af597af07557.herokuapp.com/v1/cities?pageSize=500&page=0&countryId=${encodeURIComponent(countryId)}&stateId=${encodeURIComponent(stateId)}&shippingCompanyId=${encodeURIComponent(shippingCompanyId)}`
    };
    ...
    function enableInputs() {
      (STEP || document).querySelectorAll("input,select,textarea").forEach(el => (el.disabled = false));
    }
    ...
    function fillSelect(select, list, { placeholder, valueKey = "id" } = {}) {
      ...
      opt0.value = "";
      opt0.textContent = placeholder || (select.id === "user_country"
      ...
      list.forEach(item => {
        ...
        opt.textContent = labelFrom(item) || String(item.code || item.id || "");
        select.appendChild(opt);
      });
  
      const preset = select.getAttribute("data-selected");
      const canUsePreset = preset && Array.from(select.options).some(o => o.value === preset);
      const canUsePrev   = prev && Array.from(select.options).some(o => o.value === prev);
    ...
    function findKSAId(countries) {
      const byCode = countries.find(c => String(c.code || "").toUpperCase() === "SA");
    ...
    function companyId() {
      return String(window.selectedCompany?.id || window.selectedCompany?._id || "");
    }
    function method() {
    ...
    async function loadCountries() {
      ...
      try {
        ...
        fillSelect(el, items, { placeholder: window.translations?.select_country });
        ...
        fillSelect(el, [], { placeholder: window.translations?.no_countries_found || "No countries" });
      }
    }
  
    async function loadStates() {
      const el = $state(), countryEl = $country();
      ...
      if (!cid || !comp) {
        fillSelect(el, [], { placeholder: window.translations?.select_state });
        return;
      }
      ...
      try {
        ...
        const items = Array.isArray(data?.results) ? data.results : (Array.isArray(data) ? data : []);
        fillSelect(el, items, { placeholder: window.translations?.select_state });
        if (items.length === 1) el.value = String(items[0].id);
        await loadCities();
      } catch {
        fillSelect(el, [], { placeholder: window.translations?.no_states_found || "No states" });
        fillSelect($city(), [], { placeholder: window.translations?.no_cities_available || "No cities" });
      }
    }
  
    async function loadCities() {
      const el = $city(), s = $state(), c = $country();
      if (!el || !s || !c) return;
      ...
      if (!cid || !sid || !comp) {
        fillSelect(el, [], { placeholder: window.translations?.select_city });
        return;
      }
      ...
      try {
        ...
        const items = Array.isArray(data?.results) ? data.results : (Array.isArray(data) ? data : []);
        fillSelect(el, items, { placeholder: window.translations?.select_city });
        if (items.length === 1) el.value = String(items[0].id);
      } catch {
        fillSelect(el, [], { placeholder: window.translations?.no_cities_available || "No cities" });
      }
    }
  
    function bindLocationChangeHandlers() {
      const c = $country(), s = $state();
      if (c && !c.dataset.bound) { c.addEventListener("change", loadStates); c.dataset.bound = "1"; }
      if (s && !s.dataset.bound) { s.addEventListener("change", loadCities); s.dataset.bound = "1"; }
    }
  
    function handleCompanyRequirements() {
      enableInputs();
      [$country(), $state(), $city()].forEach(el => { if (el) { el.required = true; el.disabled = false; } });
  
      const note = document.getElementById("language-note");
      if (note) {
        if (window.selectedCompany?.isEnglish === true) {
          note.style.display = "block";
          note.textContent = window.translations?.enter_in_english || "Please enter data in English only";
        } else {
    ...
    document.addEventListener("DOMContentLoaded", () => {
      const root = STEP || document;
      root.querySelectorAll("#step-3 input, #step-3 select, #step-3 textarea").forEach(el => {
        if (!el.dataset.boundStep3Live) {
          el.addEventListener("input", () => {
            if (typeof window.hardEnableNext === "function") window.hardEnableNext(validateStep3Form());
          });
